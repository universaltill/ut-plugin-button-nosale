#!/usr/bin/env bash
# Validates the button-plugin manifest: marketplace-required fields
# (id/name/semver/permissions/locales), ADR-0001 runtime, and exactly one
# type="button" entry with a trigger_event.
set -euo pipefail
cd "$(dirname "$0")/.."
python3 - <<'PY'
...
if m.get("canonical_type") != "button": errs.append("canonical_type must be 'button'")
btns = [e for e in m.get("entries", []) if e.get("type") == "button"]
if len(btns) != 1:
    errs.append(f"expected exactly 1 button entry, got {len(btns)}")
else:
    if not btns[0].get("key"): errs.append("button entry missing key")
    if not btns[0].get("label"): errs.append("button entry missing label")
    if not btns[0].get("trigger_event"): errs.append("button entry missing trigger_event")
if errs:
